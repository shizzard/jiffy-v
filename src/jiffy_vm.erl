...
-type jv_type_integer() :: #{type := integer, validator := jv_fun()}.
-type jv_type_float() :: #{type := float, validator := jv_fun()}.
-type jv_type_string() :: #{type := string, validator := jv_fun()}.
-type jv_type_boolean() :: #{type := boolean, validator := jv_fun()}.
-type jv_type_null() :: #{type := null, validator := jv_fun()}.
...
-type jv_type_hash() :: #{type := hash, fields := list(Field :: jv_type_hashfield()), validator := jv_fun()}.
-type jv_type_list() :: #{type := list, variants := list(Variant :: jv_type()), validator := jv_fun()}.
-type jv_type_enum() :: #{type := enum, variants := list(Variant :: term()), validator := jv_fun()}.
-type jv_type_variant() :: #{type := variant, variants := list(Variant :: jv_type()), validator := jv_fun()}.
-type jv_type_composite() :: jv_type_hash() | jv_type_list() | jv_type_enum() | jv_type_variant().
-type jv_type_any() :: #{type := any, validator := jv_fun()}.
...
-spec integer(Validator :: jv_fun()) ->
    Ret :: jv_type_integer().
integer(Validator) ->
    #{type => integer, validator => Validator}.
...
-spec float(Validator :: jv_fun()) ->
    Ret :: jv_type_float().
float(Validator) ->
    #{type => float, validator => Validator}.
...
-spec string(Validator :: jv_fun()) ->
    Ret :: jv_type_string().
string(Validator) ->
    #{type => string, validator => Validator}.
...
-spec boolean(Validator :: jv_fun()) ->
    Ret :: jv_type_boolean().
boolean(Validator) ->
    #{type => boolean, validator => Validator}.
...
hashfield(FieldName, IsRequired, Type, Validator) ->
    #{fieldname => FieldName, obligatoriness => IsRequired, type => Type, validator => Validator}.
...
hash(Fields, Validator) ->
    #{type => hash, fields => Fields, validator => Validator}.
...
list(Variants, Validator) ->
    #{type => list, variants => Variants, validator => Validator}.
...
enum(Variants, Validator) ->
    #{type => enum, variants => Variants, validator => Validator}.
...
variant(Variants, Validator) ->
    #{type => variant, variants => Variants, validator => Validator}.
...
handle(#{type := hash, fields := Fields, validator := Validator}, Data0, Errors0, Stack0) when is_map(Data0) ->
...
handle(#{type := hash, fields := _Fields, validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_HASH, Errors0, Stack0);

handle(#{type := list, variants := [Variant | Variants], validator := Validator}, Data0, Errors0, Stack0) when is_list(Data0) ->
    ...
    case Result of
        failure ->
            handle(#{type => list, variants => Variants, validator => Validator}, Data0, Errors0, Stack0);
        {_Index, Result1, Errors1, _Stack1} ->
            val(Validator, lists:reverse(Result1), Errors1, Stack0)
    end;

handle(#{type := list, variants := [], validator := Validator}, Data0, Errors0, Stack0) when is_list(Data0) ->
    fix(Validator, Data0, ?INVALID_LIST, Errors0, Stack0);

handle(#{type := list, variants := _Variants, validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_LIST, Errors0, Stack0);

handle(#{type := enum, variants := Variants, validator := Validator}, Data0, Errors0, Stack0) ->
...
handle(#{type := variant, variants := [Variant | Variants], validator := Validator}, Data0, Errors0, Stack0) ->
    case handle(Variant, Data0, Errors0, Stack0) of
        {error, _E1, _R1} ->
            handle(#{type => variant, variants => Variants, validator => Validator}, Data0, Errors0, Stack0);
        Res -> Res
    end;

handle(#{type := variant, variants := [], validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_VARIANT, Errors0, Stack0);

handle(#{type := string, validator := Validator}, Data0, Errors0, Stack0) when is_binary(Data0) ->
    val(Validator, Data0, Errors0, Stack0);
handle(#{type := string, validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_STRING, Errors0, Stack0);

handle(#{type := integer, validator := Validator}, Data0, Errors0, Stack0) when is_integer(Data0) ->
    val(Validator, Data0, Errors0, Stack0);
handle(#{type := integer, validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_INTEGER, Errors0, Stack0);

handle(#{type := float, validator := Validator}, Data0, Errors0, Stack0) when is_float(Data0); is_integer(Data0) ->
    val(Validator, Data0, Errors0, Stack0);

handle(#{type := float, validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_FLOAT, Errors0, Stack0);

handle(#{type := any, validator := Validator}, Data0, Errors0, Stack0) ->
    val(Validator, Data0, Errors0, Stack0);

handle(#{type := boolean, validator := Validator}, Data0, Errors0, Stack0) when is_boolean(Data0) ->
    val(Validator, Data0, Errors0, Stack0);

handle(#{type := boolean, validator := Validator}, Data0, Errors0, Stack0) ->
    fix(Validator, Data0, ?INVALID_BOOLEAN, Errors0, Stack0);

handle(#{type := null, validator := Validator}, null, Errors0, Stack0) ->
    val(Validator, null, Errors0, Stack0);

handle(#{type := null, validator := Validator}, Data0, Errors0, Stack0) ->
...
iterate_hash(#{fieldname := FName, obligatoriness := Obligatoriness, type := Type, validator := Validator}, {D0, R0, E0, S0}) ->
    case maps:find(FName, D0) of
        %% required field is unset, we're trying to fix it
        error when required == Obligatoriness ->
            case fix(Validator, undefined, ?UNDEFINED_FIELD, E0, [FName | S0]) of
                {ok, E1, R1} ->
